...
def clean_text(text):
    ...
    start_marker = "*** START OF THE PROJECT GUTENBERG EBOOK ENGLISH FAIRY TALES ***"
    end_marker = "*** END OF THE PROJECT GUTENBERG EBOOK ENGLISH FAIRY TALES ***"
...
def create_pages(paragraphs, lines_per_page=7):
    ...
    for p in paragraphs:
        # Fairy tales have short paragraphs usually.
...
def format_pages_html(pages_data, start_page_num):
    ...
    # English Fairy Tales headers are usually ALL CAPS titles of stories.
    # e.g., "TOM TIT TOT", "THE THREE SILLIES"
    # But also "Notes" at the end.
    ...
    header_pattern = re.compile(r'^[A-Z\s\W\'\-]{3,50}$')
    ...
    for i, page_lines in enumerate(pages_data):
        ...
        if page_lines:
            # Check if it matches pattern AND isn't just common words like "THE END" inside a story (though THE END is usually fine as a header)
            match = header_pattern.match(page_lines[0])
            
            # Refine checking: isupper() is safer for strict all-caps
            is_header = page_lines[0].strip().isupper() and len(page_lines[0].strip()) > 3
...
def main():
    ...
    url = "https://www.gutenberg.org/cache/epub/7439/pg7439.txt"
    ...
    for i in range(num_vols):
        ...
        base_title = "English Fairy Tales"
        ...
        volumes.append({
            ...
            "description": "Classic English folk tales.",
            "pages": vol_pages
        })

    # Output to JSON file
    with open("js/fairy_tales_books.js", "w", encoding="utf-8") as f:
        for book in volumes:
            f.write(f"window.BOOK_DATA['A1'].push({json.dumps(book, indent=4, ensure_ascii=False)});\n\n")
            
    print("Successfully generated js/fairy_tales_books.js")
